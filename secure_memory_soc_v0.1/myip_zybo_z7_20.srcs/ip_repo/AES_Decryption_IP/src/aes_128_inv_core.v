    ...
    always @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            ...
            case (state)
                ...
                ROUND_OP: begin
                    // 사이클 A: InvShiftRows + InvSubBytes + AddRoundKey → pipe_reg
                    // inv_sbox_out 안에 이미 InvShiftRows(state_reg) 포함
                    pipe_reg <= inv_sbox_out ^ round_key;
                    state    <= ROUND_MIX;    // ★ 다음 사이클로
                end
                
                ROUND_MIX: begin              // ★ 새 상태
                    // 사이클 B: InvMixColumns만 처리
