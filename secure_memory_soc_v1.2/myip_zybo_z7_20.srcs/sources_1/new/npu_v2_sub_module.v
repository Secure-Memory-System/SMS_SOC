    ...
    always @(posedge clk) begin
        if (reset_p) begin
            ...
            if (valid_buf && busy) begin
                input_reg[0] <= value_00; input_reg[1] <= value_01; input_reg[2] <= value_02;
                input_reg[3] <= value_03; input_reg[4] <= value_04; input_reg[5] <= value_05;
                input_reg[6] <= value_06; input_reg[7] <= value_07; input_reg[8] <= value_08;
                input_pending <= 1;
            end
            ...
            if (valid_buf && !busy) begin
                ...
                if (k <= 8) begin
                    ...
                    conv_out_0 <= ($signed(acc0)+$signed(bias[0]) < 0) ? 20'sd0 : $signed(acc0)+$signed(bias[0]);
                    conv_out_1 <= ($signed(acc1)+$signed(bias[1]) < 0) ? 20'sd0 : $signed(acc1)+$signed(bias[1]);
                    conv_out_2 <= ($signed(acc2)+$signed(bias[2]) < 0) ? 20'sd0 : $signed(acc2)+$signed(bias[2]);
                    conv_out_3 <= ($signed(acc3)+$signed(bias[3]) < 0) ? 20'sd0 : $signed(acc3)+$signed(bias[3]);
                    conv_out_4 <= ($signed(acc4)+$signed(bias[4]) < 0) ? 20'sd0 : $signed(acc4)+$signed(bias[4]);
                    valid_out_calc <= 1;
                    k <= 0;

                    if (input_pending) begin
                        value_buf[0] <= input_reg[0]; value_buf[1] <= input_reg[1];
                        value_buf[2] <= input_reg[2]; value_buf[3] <= input_reg[3];
                        value_buf[4] <= input_reg[4]; value_buf[5] <= input_reg[5];
                        value_buf[6] <= input_reg[6]; value_buf[7] <= input_reg[7];
                        value_buf[8] <= input_reg[8];
                        acc0 <= 0; acc1 <= 0; acc2 <= 0; acc3 <= 0; acc4 <= 0;
                        input_pending <= 0;
                        busy <= 1;
                    end else begin
            ...
            case (state)
                ...
                S_NEXT: begin
                    ...
                    if (n_ptr == 3'd4) begin
                        ...
                        rd_addr <= 8'd0;
                        mem_sel <= 3'd0;
                        // ✅ [BUG FIX] 비트 폭 명시: n_ptr(3bit)+1 의 곱셈 오버플로우 방지
                        w_addr  <= ({10'd0, n_ptr} + 13'd1) * 13'd845;
                        state   <= S_CALC;
                    end
                end
...
module npu_output_layer(
    ...
    reg signed [27:0] p0, p1, p2, p3, p4;
    reg signed [27:0] s01, s23, s4b;
    reg signed [27:0] s03;
    ...
    always @(posedge clk) begin
        if (reset_p) begin
            state <= S_IDLE; n_ptr <= 0; final_digit <= 0; result_valid <= 0;
            ...
            case (state)
                S_IDLE: begin
                    result_valid <= 0;
                    if (dense_done) state <= S_FETCH;
                end

                S_FETCH: begin
                    tw0 <= w1_rom[(n_ptr << 2) + n_ptr + 0];
                    tw1 <= w1_rom[(n_ptr << 2) + n_ptr + 1];
                    tw2 <= w1_rom[(n_ptr << 2) + n_ptr + 2];
                    tw3 <= w1_rom[(n_ptr << 2) + n_ptr + 3];
                    tw4 <= w1_rom[(n_ptr << 2) + n_ptr + 4];
                    state <= S_MULT;
                end

                S_MULT: begin
                    p0 <= d_in_0 * tw0; p1 <= d_in_1 * tw1;
                    p2 <= d_in_2 * tw2; p3 <= d_in_3 * tw3;
                    p4 <= d_in_4 * tw4;
                    state <= S_ADD_1;
                end
                ...
                S_ADD_3: begin
                    ...
                    if (n_ptr == 9) begin
                        n_ptr <= 0; state <= S_ARGMAX;
                    end else begin
                        n_ptr <= n_ptr + 1; state <= S_FETCH;
                    end
                end
                ...
                S_ARGMAX: begin
                    ...
                    if (n_ptr == 9) begin
                        n_ptr <= 0; max_score <= 28'sh8000000;
