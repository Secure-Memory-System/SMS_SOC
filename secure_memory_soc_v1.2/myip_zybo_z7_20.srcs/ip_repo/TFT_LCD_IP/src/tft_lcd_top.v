...
module tft_lcd_top(
    ...
    always @(posedge clk or posedge reset_p) begin
        if (reset_p) begin
            ...
            // 좌표 획득(Get_Flag) 시 9클럭 동안 3x3 픽셀 도장 찍기 시작!
            if (Get_Flag && ~pen_lock) begin
                brush_state <= 1;
            end else if (brush_state > 0) begin
                case (brush_state)
                    1: begin brush_addr <= (gy_m * 10'd28) + gx_m; brush_wr_en <= 1; brush_state <= 2; end
                    2: begin brush_addr <= (gy_m * 10'd28) + gx;   brush_wr_en <= 1; brush_state <= 3; end
                    3: begin brush_addr <= (gy_m * 10'd28) + gx_p; brush_wr_en <= 1; brush_state <= 4; end
                    4: begin brush_addr <= (gy   * 10'd28) + gx_m; brush_wr_en <= 1; brush_state <= 5; end
                    5: begin brush_addr <= (gy   * 10'd28) + gx;   brush_wr_en <= 1; brush_state <= 6; end
                    6: begin brush_addr <= (gy   * 10'd28) + gx_p; brush_wr_en <= 1; brush_state <= 7; end
                    7: begin brush_addr <= (gy_p * 10'd28) + gx_m; brush_wr_en <= 1; brush_state <= 8; end
                    8: begin brush_addr <= (gy_p * 10'd28) + gx;   brush_wr_en <= 1; brush_state <= 9; end
                    9: begin brush_addr <= (gy_p * 10'd28) + gx_p; brush_wr_en <= 1; brush_state <= 0; end
