...
/**
 * lcd_bram_dp
 * 기능: 28x28 (784바이트) 듀얼포트 BRAM
 *
 * Port A: LCD 표시용 읽기 + 터치/클리어 쓰기 (tft_lcd_top 내부)
 * Port B: write_master 전용 읽기 (외부에서 BRAM_0으로 복사할 때)
 *
 * 기존 lcd_bram과 차이점:
 *   - DEPTH: 320*240 → 28*28 (784)
 *   - 주소 폭: 17bit → 10bit
 *   - 싱글포트 → 듀얼포트 (Port B 읽기 전용 추가)
 */
module lcd_bram_dp #(
    parameter WIDTH = 8,
    ...
    // =========================================================
    // Port B: write_master 읽기 전용 (32-bit로 확장)
    // =========================================================
    input  wire               clkb,
    input  wire               enb,
    input  wire [7:0]         addrb,        // 0 ~ 195 (32-bit Word 주소)
    output reg  [31:0]        doutb         // 4픽셀 패킹된 32-bit 데이터
);
    ...
    always @(posedge clkb) begin
        if (enb) begin
            // 8-bit 픽셀 4개를 한 번에 읽어서 32-bit로 묶기 (Little Endian 방식)
            doutb <= {ram[{addrb, 2'b11}], 
                      ram[{addrb, 2'b10}], 
                      ram[{addrb, 2'b01}], 
                      ram[{addrb, 2'b00}]};
        end
