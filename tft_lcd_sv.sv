...
module spi(
    input clk, reset_p,
    input[8:0] data,        // 최상위 비트는 명렁어 or 데이터 인지 결정
    ...
    wire[0:7] dataShift = internalData[7:0]; // 최상위 비트 부터 보냄
        ...
        if(reset_p)begin
            ...
            if (!idle)begin
                ...
                if (internalSck) begin
                    ...
                    //idle <= &counter; // 각 비트 자리 수 전체를 & masking glitch 발생할 수 있어서 잘 사용 안함
                    if(counter == 7) idle <= 1;
                end
            end
            else begin
...
module tft_lcd_top(
    ...
    reg [10:0] wr_addr, rd_addr;  // 28*28=784
    reg [7:0] data_to_ram;
    ...
    wire [7:0] cnt_x_off = cnt_x - 8;    // 0~223
    wire [8:0] cnt_y_off = cnt_y - 48;   // 0~223
    ...
    always @(posedge framebufferClk, posedge reset_p) begin
        if(reset_p) begin
            ...
            if(cnt_x >= 8 && cnt_x < 232 &&
               cnt_y >= 48 && cnt_y < 272) begin
                // 8×8 스케일: >>3
                rd_addr <= cnt_y_off[8:3] * 28 + cnt_x_off[7:3];
            end
            else rd_addr <= 0;
    
            if(cnt_x >= 240-1) begin
                cnt_x <= 0;
                if(cnt_y >= 320-1) cnt_y <= 0;
                else cnt_y <= cnt_y + 1;
            end
            else cnt_x <= cnt_x + 1;
        end
    ...
    // penx[15:8] 범위 0~255 → LCD X 근사
    // peny[15:6] 범위 0~255 → LCD Y 근사
    // LCD 중앙 112×112: X 64~175, Y 104~215
    // BRAM 28×28: X /4 → 0~27, Y /4 → 0~27
    wire [7:0] penx_lcd = penx[15:8];
    wire [7:0] peny_lcd = peny[15:6];

    wire [7:0] penx_off = penx_lcd - 8;
    wire [7:0] peny_off = peny_lcd - 48;
    ...
    always @(posedge clk, posedge reset_p) begin
        if(reset_p) begin
            ...
            if(penx_lcd >= 8  && penx_lcd < 232 &&
               peny_lcd >= 48 && peny_lcd < 272) begin
                // 8×8 스케일: >>3
                wr_addr     <= peny_off[7:3] * 28 + penx_off[7:3];
                data_to_ram <= 8'hff;
            end
        end
    end
    ...
    wire [15:0] px;
    bin_to_dec btdx(.bin(penx), .bcd(px));
    FND_ctr fnd(
        .clk(clk),
        .reset_p(reset_p),
        .fnd_value(px),
...
module lcd_bram
    #(parameter WIDTH = 8, parameter DEPTH = 320*240)
    (
    input wclk,
    input wr_en,
    input [16:0] wr_addr,
